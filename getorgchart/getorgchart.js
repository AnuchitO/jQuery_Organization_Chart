getOrgChart = function(b, a) {
	...
	this.element = b;
	this.render = (this.config.render == "AUTO") ? getOrgChart._C()
			: this.config.render;
	this._au = [];
	this._aj = [];
	this._aO = [];
...
getOrgChart.prototype._az = function() {
	this._zA = get._r().msie ? this.element.clientWidth : window
	...
	this._zQ = get._r().msie ? this.element.clientHeight : window
	...
	var a = getOrgChart.INNER_HTML.replace("[theme]", this.config.theme)
			.replace("[color]", this.config.color).replace(/\[height]/g,
					this._aF).replace(/\[toolbar-height]/g,
...
getOrgChart.prototype._p = function() {
	...
	for (var b = 0; b < this._aB.length; b++) {
		...
		case "SVG":
			...
			var showRank = d.data.rank ;
			var showDepartment = d.data.department;
			var h = a ? this.theme.textPoints : this.theme.textPointsNoImage;

//========================================= set show rank and department================================================
			f.push(getOrgChart.OPEN_GROUP.replace("[x]", j).replace("[y]", l)
					.replace("[level]", d.level).replace("[rank]",showRank+"\n").replace("[department]",showDepartment));
//========================================= set show rank and department================================================
...
getOrgChart.prototype.draw = function() {
	...
	if (this.config.zoomable) {
		this._Q._zC.style.display = "none";
		this._Q._zE.style.display = "none"
	}
	...
	if (this.config.printable) {
		this._Q._aL.style.display = "none"
	}
	if (this.config.movable) {
		this._Q._a6.style.display = "none";
		this._Q._ay.style.display = "none";
		this._Q._o.style.display = "none";
		this._Q._zo.style.display = "none"
	}
...
//============================= show rank and department ==========================================================
getOrgChart.OPEN_GROUP = '<g class="get-level-[level]" title="Rank: [rank]Department: [department]" transform="matrix(1,0,0,1,[x],[y])">';
...
getOrgChart._s = function(u, n, g) {
	...
	for (var d = u.config.maxDepth - g; a != null && b != null && c <= d;) {
		...
		var t = (b._aM + h + u._L(b) + u.config.subtreeSeparation)
				- (a._aM + m);
		if (t > 0) {
			var r = n;
			var o = 0;
...
getOrgChart._zd = function(h, d, b, j, l) {
	if (b <= h.config.maxDepth) {
		...
		if (d._V() != 0) {
			getOrgChart._zd(h, d._J(), b + 1, j + d._am, l + c
					+ h.config.levelSeparation)
		}
		var f = d._P();
		if (f != null) {
...
getOrgChart._zk = function(a) {
	for (i = 0; i < a._zu.length; i++) {
		...
		while (b > c) {
			a._zu[i].textContent = a._zu[i].textContent.substring(0,
					a._zu[i].textContent.length - 4);
			a._zu[i].textContent += "...";
...
getOrgChart.person.prototype.compareTo = function(b) {
	var a = this;
	if (a === undefined || b === undefined || a._zW === undefined
			|| a._zS === undefined || b._zW === undefined
			|| b._zS === undefined) {
		return false
	} else {
...
getOrgChart.person.prototype._l = function(d) {
	...
	for (var a = 0; a < this._aZ.length; a++) {
		...
		case "SVG":
			...
			case "M":
				c.push('<path class="link"   d="M' + e + "," + i + " " + f
						+ "," + j + " " + g + "," + l + " L" + h + "," + m
						+ '"/>');
				break;
			case "B":
				c.push('<path class="link"  d="M' + e + "," + i + " C" + f
						+ "," + j + " " + g + "," + l + " " + h + "," + m
						+ '"/>');
				break
			}
...
get._w = function(a, c, b, h, j, d) {
	...
	function f() {
		for ( var s in b) {
			var t = getOrgChart.util._zZ([ "top", "left", "right", "bottom" ],
					s.toLowerCase()) ? "px" : "";
...
getOrgChart.prototype._zg = function(c) {
	...
	if (e) {
		this._Q._n.innerHTML = getOrgChart.DETAILS_VIEW_ID_IMAGE.replace(
				"[src]", e)
	} else {
...
getOrgChart.prototype._ak = function(f, d) {
	...
	if (this._aR.dragStart) {
		...
		var e = g[3] / this._aF;
		var c = h > e ? h : e;
		g[0] = -((this._aR.mouseLastX - this._aR.dragStart.x) * c)
				+ this._aR.dragStart.viewBoxLeft;
		g[1] = -((this._aR.mouseLastY - this._aR.dragStart.y) * c)
				+ this._aR.dragStart.viewBoxTop;
		g[0] = parseInt(g[0]);
		g[1] = parseInt(g[1]);
...
getOrgChart.prototype._ai = function(b, a) {
	...
	this._aR.dragStart = {
		x : this._aR.mouseLastX,
		y : this._aR.mouseLastY,
		viewBoxLeft : c[0],
		viewBoxTop : c[1]
	};
...
getOrgChart.prototype._ab = function(i) {
	...
	if (this._zw.found.length) {
		...
		get._w(this._Q._f, {
			...
			viewBox : o
		}, 150, get._w._aC, function() {
			get._w(b, {
				transform : g
			}, {
				transform : k
			}, 200, get._w._aX, function() {
				get._w(b, {
					transform : k
				}, {
					transform : g
...
getOrgChart.prototype._k = function() {
	if ((this._zw.showIndex < this._zw.found.length - 1)
			&& (this._zw.found.length != 0)) {
...
getOrgChart.prototype._W = function(e) {
	...
	for (n = 0; n < this._aB.length; n++) {
		for (m in this._aB[n].data) {
			...
			var b = -1;
			if (getOrgChart.util._af(this._aB[n])
					&& getOrgChart.util._af(this._aB[n].data[m])) {
...
getOrgChart.prototype.updatePerson = function(b, c, a) {
	...
	if (b == "") {
		...
		for (t = this._aB.length - 1; t >= 0; t--) {
			if (this._aB[t].id == b) {
				if (this._aB[t].pid == null
						|| typeof (this._aB[t].pid) == "undefined"
						|| this._aB[t].pid == "") {
					this._aB[t].data = a
				} else {
					if (this._aB[t].pid == c) {
